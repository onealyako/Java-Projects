...
public class MyConnection {
    ...
    public void addCourseAttempt(Student s, Attempt a){
		System.out.println("Saving Attempt");
		String semesterTake = a.getSemesterTaken();
        String courseStatus = a.getCourseStatus();
        String grade = a.getCourseGrade();
        String courseName = a.getCourse().getCourseTitle();
		String id = a.getCourse().getCourseCode();
        String name = s.getFirstName();
        String studentID = s.getStudentNum().toString();
		String degree = s.getDegreeProgram().getDegreeTitle();

        String courseString = id + "," + semesterTake + "," + grade + "," + courseStatus;

        System.out.println("Inserting...");

        String sql = "INSERT INTO SavedStudent(id,name,degree,course) VALUES('" + studentID + "','" + name + "','" + degree + "','" + courseString + "');";
        System.out.println(sql);
        dbUpdate(sql);

	}

    public void addCourseFuture(Student s, Course c){
		System.out.println("Saving Future Cource");
		String semesterTake = c.getSemesterOffered();
        String weight = c.getCourseCredit() + "";
        String courseName = c.getCourseTitle();
		String id = c.getCourseCode();
        String name = s.getFirstName();
        String studentID = s.getStudentNum().toString();
		String degree = s.getDegreeProgram().getDegreeTitle();

        String courseString = id + "," + weight + "," + courseName + "," + semesterTake + "," + "prereqs";
	...
	public DBStudent loadStudent(String id, String name){
    	...
    	public DBStudent findStudent(String id){
		System.out.println("Returning saved student object");
		DBStudent s = new DBStudent(); //what will be returned
		ArrayList<String> cList = new ArrayList<String>(); //where we store the courses

		String sql = "SELECT * FROM SavedStudent WHERE id = '" + id + "';" ;
        connect();
    ...
    public void changeCourseGrade(Student s, String id, String semester, String newGrade) {

        String sql = "UPDATE `SavedStudent` set course = concat(SUBSTRING_INDEX(course, ',', 1), ',', SUBSTRING_INDEX(SUBSTRING_INDEX(course, ',', 2), ',', -1), ',', '" + newGrade + "', ',', SUBSTRING_INDEX(SUBSTRING_INDEX(course, ',', 4), ',', -1) ) WHERE SUBSTRING_INDEX(course, ',', 1) = '" + id + "' and SUBSTRING_INDEX(SUBSTRING_INDEX(course, ',', 2), ',', -1) = '" + semester + "' and id = '" + s.getStudentNum() + "'";
        System.out.println("Pushing Grade Change to DB");
        System.out.println(sql);
        dbUpdate(sql);
    }

    public void deleteStudentCourse(Student s, String id, String semester) {

        String sql = "DELETE FROM `SavedStudent` WHERE SUBSTRING_INDEX(course, ',', 1) = '" + id + "' and id = '" + s.getStudentNum() + "'";
